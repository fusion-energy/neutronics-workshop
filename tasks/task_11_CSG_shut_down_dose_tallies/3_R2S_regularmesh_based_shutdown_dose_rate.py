...
mixed_materials_in_each_mesh_voxel = regular_mesh.get_homogenized_materials(
    model_neutron, n_samples=1_000_000
)
...
results = openmc.deplete.Results(
    statepoints_folder / "neutrons" / "depletion_results.h5"
)
...
for i_cool in range(1, len(timesteps)):
    ...
    for activated_mat_id in activated_mat_ids:
        ...
        if strength > 0.0:
            source = openmc.IndependentSource(
                energy=energy, particle="photon", strength=strength
            )
        else:
            print("source has no gammas")
            source = (
                openmc.IndependentSource()
            )  # how to make an empty source, source strength is set to 0
    ...
    model_gamma = openmc.Model(
        my_geometry, my_materials, my_gamma_settings, my_gamma_tallies
    )
...
for i_cool in range(
    1, len(timesteps)
):  # skipping the first depletion step as we just want the part where the machine is off for the shut down dose rate
    with openmc.StatePoint(
        statepoints_folder
        / "photons"
        / f"photon_at_time_{i_cool}"
        / f"statepoint.{my_gamma_settings.batches}.h5"
    ) as statepoint:
        ...
        print(
            "max",
            (max(photon_tally.mean.flatten()) * scaling_factor)
            / mesh_photon.volumes[0][0][0],
        )
        print(
            "min",
            (min(photon_tally.mean.flatten()) * scaling_factor)
            / mesh_photon.volumes[0][0][0],
        )
        ...
        plot.title.set_text(
            f"timestep {sum(timesteps[1:i_cool])/(60*60)} hours after shut down"
        )
