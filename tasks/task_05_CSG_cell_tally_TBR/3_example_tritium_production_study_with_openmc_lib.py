...
my_settings.trigger_max_batches = (
    100  # this is maximum number of batches that will be run
)
...
tbr_tally.scores = [
    "H3-production"
]  # Could also use '(n,Xt)' where X is a wildcard character
...
for enrichment in enrichments:  # percentage enrichment from 0% Li6 to 100% Li6
    ...
    breeder_material.add_nuclide("Li7", 15.8 * (1.0 - enrichment))
    ...
    print(
        f"old nuclides {lib_breeder_material.nuclides}"
    )  # List of nuclides in the material
    print(
        f"old nuclides {lib_breeder_material.densities}"
    ...
    print(
        f"new nuclides {lib_breeder_material.nuclides}"
    )  # List of nuclides in the material
    print(
        f"new nuclides {lib_breeder_material.densities}"
    )  # Array of densities in atom/b-cm
