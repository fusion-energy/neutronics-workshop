...
#therefore we use these shortened lists instead
candidate_fusion_neutron_multipliers_list = ['Be9', 'Pb204', 'Pb206'] # , 'Pb207','Pb208']
candidate_fusion_tritium_producers_list = ['Li6','Li7']

# these are the MT numbers for the (n,2n) and (n,Xt) reactions, where X is a wildcard. Other reactions can be found at https://www.oecd-nea.org/dbdata/data/manual-endf/endf102_MT.pdf
neutron_multiplication_MT_number = 16
tritium_production_MT_number = 205
...
# this loop plots n,2n cross-sections for all isotopes in the candidate_fusion_neutron_multipliers_list
...
for isotope_name in tqdm(candidate_fusion_neutron_multipliers_list):
      ...
      if neutron_multiplication_MT_number in isotope_object.reactions.keys():
            cross_section = isotope_object[neutron_multiplication_MT_number].xs['294K'](energy)
            fig.add_trace(go.Scatter(x=energy,
                              y=cross_section,
                              mode = 'lines',
                              name=isotope_name+' MT '+ str(neutron_multiplication_MT_number)
                              )
                        )
      else:
            print('isotope ', isotope_name , ' does not have the MT reaction number ',neutron_multiplication_MT_number)


# this loop plots n,Xt cross-sections for all isotopes in the candidate_fusion_tritium_producers_list

# for isotope_name in tqdm(candidate_fusion_tritium_producers_list):
#       isotope_object = openmc.data.IncidentNeutron.from_hdf5(os.path.join(nuclear_data_path,isotope_name+'.h5')) # you may have to change this directory
#       energy = isotope_object.energy['294K'] # 294K is the temperature for endf, others use 293K
#       if tritium_production_MT_number in isotope_object.reactions.keys():
#             cross_section = isotope_object[tritium_production_MT_number].xs['294K'](energy)
#             fig.add_trace(go.Scatter(x=energy,
#                               y=cross_section,
#                               mode = 'lines',
#                               name=isotope_name+' MT '+ str(tritium_production_MT_number)
#                               )
#                         )
#       else:
#             print('isotope ', isotope_name , ' does not have the MT reaction number ',tritium_production_MT_number)


fig.update_layout(
      title = 'Isotope cross sections',
